...
extension WBSQLiteManager {
    //清理缓存
    // - 但是，如果删除了数据，数据库的大小，不会变小，既然曾经有这么大，就不会改变数据库的大小
    // 如果要变小，
    // 将数据库文件复制一个新的副本，status.db.old
    //新建一个空的数据库文件，
    //自己编写sql。从old中将所有数据读出，写入新的数据库 ,删除old数据库
    @objc private func clearDBCache() {
        let dateStr = Date.cz_dateString(delta: TimeInterval(maxDBCacheTime))
